...
## Part 3: Test the ICS Integration using SoapUI
...
**3.2** Click on the *REST* button so we can create a new project for testing our new ICS REST Web Service

![](images/200/image061.png)

**3.3** In the **New REST Project** dialog window, paste the REST Service link you copied earlier into the **URI** window. 

**3.4** _Important_: Replace the string `metadata` with the service endpoint `NetSuiteQuery`

**3.5** Click on the *OK* button after you've set the URI for your new SoapUI REST project.

![](images/200/image062.png)

**3.4** The new REST Project will appear in the left-hand navigation.

**3.5** Note that the auto-generated sample request **Request 1** is opened for the **NetSuiteQuery** REST operation. 

![](images/200/image063.png)

**3.6** Click on the **Method** for the request and select _POST_ from the list.
...
**3.12** We now need to enter the inbound payload for the REST API call.  Enter the following JSON payload into the 

**3.17** Finally your request payload is ready to send to ICS.  Enter the JSON payload as shown below:
...
**3.20** Click on the _JSON_ tab in the **Request 1** output window

![](images/200/image070.png)

**3.20** The return payload of the ICS integration will show the values which were looked up in the NetSuite API call.  An example of the output is shown here and in the following screenshot:
