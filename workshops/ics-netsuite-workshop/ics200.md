...
**3.1.15** Just below the raw input data textarea, the results of the ICS integration call will be displayed.  The return payload of the ICS integration will show the values which were looked up in the NetSuite API call.  Here is an example of what the return data might look like (this will vary depending on the VendorPaymentId passed in).
...
**3.2.12** Now we need to add your request payload to send to ICS.  Enter the JSON payload as shown below:
