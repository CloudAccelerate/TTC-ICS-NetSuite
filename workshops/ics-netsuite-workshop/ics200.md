...
**3.1.9** Enter your ICS credentials, then click on the *Update Request* button to add the **Authorization** header to the REST payload.  After selecting the button, you'll notice that a header is added by the *(1)* just to the right of the **Header** tab.

![](images/200/image074.png)

**3.1.10** Finally your request payload is ready to send to ICS.  Select the *Body* tab just to the right of the **Headers*.

**3.1.11** Select the *raw* data type, then select  *JSON (application/json)* from the picklist of input data Content-Types.

![](images/200/image075.png)

**3.1.12** Enter the JSON payload into the raw data textbox as shown below:

**3.1.13** Click on the blue _Send_ button to the right of the REST API URI to send the input data over to ICS..
...
**3.1.14** Just below the raw input data textarea, the results of the ICS integration call will be displayed.

**3.1.15** The return payload of the ICS integration will show the values which were looked up in the NetSuite API call.  An example of the output is shown here and in the following screenshot:
...
**3.2.4** The new REST Project will appear in the left-hand navigation.
...
**3.2.8** Click on the *Auth* button in the lower-left of the **Request 1** SoapUI window
...
**3.2.12** Finally your request payload is ready to send to ICS.  Enter the JSON payload as shown below:
...
**3.2.16** The return payload of the ICS integration will show the values which were looked up in the NetSuite API call.  An example of the output is shown here and in the following screenshot:
...
We have now tested an end-to-end integration to NetSuite using Oracle Integration Cloud Service.
