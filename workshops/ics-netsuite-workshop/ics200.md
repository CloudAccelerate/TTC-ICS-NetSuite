...
### **3.1**: Testing Using SoapUI

**3.1.1** Open SoapUI.  If you don't already have this installed, follow the instructions provided in the **Prerequisites** section of this workshop.

![](images/200/image060.png)

**3.1.2** Click on the *REST* button so we can create a new project for testing our new ICS REST Web Service

![](images/200/image061.png)

**3.1.3** In the **New REST Project** dialog window, paste the REST Service link you copied earlier into the **URI** window. 

**3.1.4** _Important_: Replace the string `metadata` with the service endpoint `NetSuiteQuery`

**3.1.5** Click on the *OK* button after you've set the URI for your new SoapUI REST project.

![](images/200/image062.png)

**3.1.4** The new REST Project will appear in the left-hand navigation.

**3.1.5** Note that the auto-generated sample request **Request 1** is opened for the **NetSuiteQuery** REST operation. 

![](images/200/image063.png)

**3.1.6** Click on the **Method** for the request and select _POST_ from the list.

![](images/200/image064.png)

**3.1.7** Next we need to add the authorization credentials so ICS will allow the request from SoapUI.  ICS uses basic username/password authentication.

**3.1.8** Click on the *Auth* button in the lower-left of the **Request 1** SoapUI window

![](images/200/image065.png)

**3.1.9** In the **Authorization** dropdown, select _Add New Authorization..._

![](images/200/image066.png)

**3.1.10** In the **Add Authorization** dialog pop-up window, select **Type** of _Basic_ form the picklist, then select the *OK* button.

![](images/200/image067.png)

**3.1.11** Fill in your assigned username and password in the **Auth (Basic)** window at the bottom of the SoapUI request window

![](images/200/image068.png)

**3.1.12** We now need to enter the inbound payload for the REST API call.  Enter the following JSON payload into the 

**3.1.17** Finally your request payload is ready to send to ICS.  Enter the JSON payload as shown below:
...
**3.1.18** Click on the green _Submit Request_ arrow in the upper left of the **Request 1** window.

![](images/200/image069.png)

**3.1.19** The right side of the **Request 1** SoapUI window will display the results of the ICS integration call.
...
**3.1.20** Click on the _JSON_ tab in the **Request 1** output window

![](images/200/image070.png)

**3.1.21** The return payload of the ICS integration will show the values which were looked up in the NetSuite API call.  An example of the output is shown here and in the following screenshot:
